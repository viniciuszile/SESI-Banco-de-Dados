...
select m.nome , m.cpf, m.id_gest_conv, g.nome, g.cargo, g.idgestor from med_convs m
inner join gestores g
on m.id_gest_conv = g.idgestor;

/*------------------------- MEDICO CONVIDADO - EXAME -------------------------*/
select m.nome as 'nome do médico convidado', m.crm, m.especialidade, e.exame, e.tipo, e.descricao from med_convs m
inner join exames e
